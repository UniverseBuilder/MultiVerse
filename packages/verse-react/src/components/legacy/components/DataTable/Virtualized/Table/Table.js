...
export default class Table extends React.PureComponent {
  ...
  _createSubColumn({
    ...
    return (
      <div
        ...
        role="gridcell"
        title={title}
        className="subColumns"
        style={{
          flex: `1 1 ${columnData.width}`,
        }}
      >
        {renderedCell}
      </div>
  ...
  _createColumn({
    ...
    if (columnData.subHeading) {
      renderedCell = columnData.subHeading.map((column, columnIndex) => (
        <React.Fragment key={"ow" + rowIndex + "-" + "ol" + columnIndex}>
          {this._createSubColumn({
            column: {
              props: {
                cellDataGetter,
                cellRenderer,
  ...
  _getFlexStyleForColumn(column, customStyle = {}) {
    const flexValue = `${column.props.flexGrow} ${column.props.flexShrink} ${column.props.width}`;
    ...
    if (column.props.resize) {
      style.resize = column.props.resize;
    }

...
class BaseTable extends React.PureComponent {
  ...
  renderEmptyLayer() {
    ...
    if ((data && data.length) || (frozenData && frozenData.length)) {
      return null;
    }
  ...
  render() {
    ...
    return (
      <div className={cls} ref={this._setContainerRef} style={containerStyle}>
        {this.renderFooter()}
  ...
  _handleRowHeightChange(rowKey, size, rowIndex) {
    if (this._resetIndex === null) {
      this._resetIndex = rowIndex;
    } else if (this._resetIndex > rowIndex) {
      this._resetIndex = rowIndex;
    }
